...
class AI:
    def __init__(self, color, difficulty):
        ...
        self.piece_values = {
            "Queen Bee": 1000,
            "Beetle": 100,  # Important for attacking and covering
            "Spider": 60,  # Good for pinning pieces
            "Soldier Ant": 150,  # Excellent mobility for surrounding
            "Grasshopper": 60,
        }
    ...
    def ai_move(self, game, tiles, tile_dict, all_tiles, all_tile_dict, depth=3):
        ...
        if self.color == "BLACK":
            for (piece, position), moves in valid_moves.items():
                for move in moves:
                    self.move_piece(piece, position, move, all_tile_dict)
                    _,value = self.iterative_deepening(game, tiles, tile_dict, all_tiles, all_tile_dict, depth, False,start_time)
                    self.undo_move(piece, position, move, all_tile_dict)
                    if value < best_value:
                        best_value = value
            ...
            (piece, position, move), value = self.iterative_deepening(game, tiles, tile_dict, all_tiles, all_tile_dict, depth, True, start_time)
        best_move = (piece, all_tile_dict[position], tile_dict[move])
        return best_move
